...
namespace Rut
{
	namespace FileX
	{
		...
		void SaveFileViaPath(const wchar_t* wpPath, void* pData, size_t nBytes)
		{
			...
			DWORD dwWritten = 0;
			WriteFile(hfile, pData, static_cast<DWORD>(nBytes), &dwWritten, NULL);
		...
		std::string GetModuleNameViaBaseA(HMODULE hModule)
		{
			...
			name.resize(GetModuleNameViaBase_Ptr((char*)name.c_str(), hModule));
			return name;
		}
		...
		std::wstring GetModuleNameViaBaseW(HMODULE hModule)
		{
			std::wstring name;
			name.resize(MAX_PATH);
			name.resize(GetModuleNameViaBase_Ptr((wchar_t*)name.c_str(), hModule));
			return name;
		}
		...
		char* FormatSlash_Ptr(char* cpPath, char cSlash)
		{
			...
			case '\\':
			{
				...
				{
					// skip the trail byte of a double-byte char so it won't be treated as an ASCII '/'
					if ((unsigned char)cpPath[ite] > 0x7F) { ite++; continue; }
		...
		size_t PathRemoveFileName_Ptr(char* cpPath)
		{
			...
			{
				if (cpPath[ite_char] == '\\' || cpPath[ite_char] == '/')
				{
					// only split at a separator that follows an ASCII char,
					// otherwise we may cut into the middle of a double-byte char
					if ((unsigned char)cpPath[ite_char - 1] <= 0x7F)
					{
						cpPath[ite_char + 1] = '\0';
						return ite_char + 1;
					}
		...
		size_t PathRemoveFileName_Ptr(wchar_t* wpPath)
		{
			...
			for (size_t ite_char = len - 1; ite_char > 0; ite_char--)
			{
				if (wpPath[ite_char] == L'\\' || wpPath[ite_char] == L'/')
				{
		...
		size_t PathGetFileName_Ptr(char* cpPath)
		{
			...
			for (size_t iteChar = len - 1; iteChar > 0; iteChar--)
			{
				...
				if ((unsigned char)cpPath[iteChar - 1] <= 0x7F)
				{
